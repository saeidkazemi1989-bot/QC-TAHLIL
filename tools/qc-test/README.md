...
| `setuptest.mjs` | «نصبِ خودکارِ openpyxl»: دسترسیِ endpoint (۴۰۱/۴۰۳)، اجرای واقعی برای مدیر سیستم، منطقِ تلاشِ دوباره (`--user` و PEP 668) با پایتونِ ساختگی، و سلامتِ `install_windows.bat`/`run_windows.bat` (CRLF، پیدا کردنِ پایتون از PATH) |
